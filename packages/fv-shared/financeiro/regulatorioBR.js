/**
 * regulatorioBR.js — motor regulatório Lei 14.300 — FV-DOM-009.
 *
 * Movido de `frontend/src/utils/financeiroRegulatorioBR.js`. Era a peça de maior
 * risco regulatório do sistema: cronograma do Fio B, direito adquirido e custo
 * de disponibilidade viviam SÓ no navegador, sem par no backend — uma mudança da
 * ANEEL exigia deploy de frontend.
 *
 * Fórmulas, cronograma, modalidades e defaults vieram INTACTOS. As decisões
 * pendentes (cenário oficial otimista × realista — D5) seguem abertas.
 *
 * FV-DOM-011C: o parâmetro morto de sazonalidade (R10) foi REMOVIDO — ver a nota
 * em `calcularRetornoRegulatorio`. Nenhum resultado mudou: ele já era descartado.
 *
 * Única alteração: o import de `financeiroEngine` ganhou a extensão `.js`, sem a
 * qual o módulo resolve no Vite mas quebra em Node ESM puro (R11) — condição
 * para o backend conseguir carregá-lo.
 *
...
import { calcularTIR } from './financeiroEngine.js'
 ...
 *
 * ⚠️ SAZONALIDADE NÃO SUPORTADA (FV-DOM-011C · R10). Este cálculo é ANUAL.
 * Havia aqui dois parâmetros — `geracaoMensalKwh` e `consumoMensalKwh` — que
 * eram normalizados em pesos e depois DESCARTADOS: nenhum resultado dependia
 * deles, e nenhum consumidor chegou a passá-los. Foram removidos.
 *
 * Não foram implementados porque a regra pretendida não está determinada em
 * lugar nenhum do código: casar geração e consumo mês a mês muda simultaneidade,
 * compensação e o rolar de créditos entre meses — decisão regulatória (D5), não
 * técnica. Anualizar superestima a compensação em perfis sazonais; isso segue
