/**
 * financeiroEngine.js — motor financeiro EPC — FV-DOM-009.
 *
 * Movido de `frontend/src/utils/` para o pacote compartilhado. Fórmulas,
 * constantes, defaults, arredondamentos e nomes de campo vieram INTACTOS — a
 * equivalência valor a valor com o `git HEAD` é verificada em
 * `financeiroConsolidacao.check.js`.
 *
 * As divergências que a FV-DOM-008A mediu (payback fracionário × inteiro,
 * ausência de VPL, inflação sem default) **continuam como estão**: resolvê-las
 * é decisão de negócio (D1–D3), não de arquitetura.
 *
 * Motor financeiro EPC. Funções puras e determinísticas — sem I/O.
 *
...
export function calcularRetorno({
  ...
  return {
    ...
    // FV-DOM-012: a série anual já era calculada aqui e descartada. Expô-la é
    // ADITIVO — nenhum campo existente mudou — e evita que o contrato V1
    // reimplemente o fluxo para descontar o VPL (invariante: uma só fórmula).
    fluxos_anuais: fluxos.map(round2),
  }
}

/**
 * TIR (IRR) anual por bisseção, COM estado de convergência — FV-DOM-011C (E7).
 *
 * `calcularTIR` devolvia `null` tanto para "não há troca de sinal no intervalo"
 * quanto para "os fluxos são inválidos" — indistinguíveis de um resultado
 * legítimo ausente. O valor NÃO mudou: esta função apenas explicita o porquê.
 *
 * @returns {{ valor_aa_pct: number|null, convergiu: boolean,
 *             motivo: 'ok'|'iteracoes_esgotadas'|'fora_do_intervalo'|'fluxos_invalidos',
 *             intervalo_busca: [number, number] }}
 */
export function calcularTIRDetalhado(fluxos, { iteracoes = 200, lo = -0.95, hi = 2 } = {}) {
  ...
  if (!Number.isFinite(fa) || !Number.isFinite(fb)) {
    return { valor_aa_pct: null, convergiu: false, motivo: 'fluxos_invalidos', intervalo_busca }
  }
  // Sem troca de sinal, a bisseção não tem raiz para encontrar: ou a TIR está
  // acima de `hi`, ou abaixo de `lo`, ou não existe.
  if (fa * fb > 0) {
    return { valor_aa_pct: null, convergiu: false, motivo: 'fora_do_intervalo', intervalo_busca }
  }
  ...
  for (let k = 0; k < iteracoes; k++) {
    ...
    const fm = npv(m)
    if (Math.abs(fm) < 1e-6) {
      return { valor_aa_pct: +(m * 100).toFixed(2), convergiu: true, motivo: 'ok', intervalo_busca }
    }
    if (fa * fm < 0) { b = m; fb = fm } else { a = m; fa = fm }
  }
  // Esgotou as iterações: o valor do ponto médio é o que sempre foi devolvido.
  // Ele É uma aproximação utilizável — `convergiu: false` diz que a tolerância
  // não foi atingida, não que o número seja inválido.
  return {
    valor_aa_pct: +(((a + b) / 2) * 100).toFixed(2),
    convergiu: false,
 ...
 *
 * Superfície preservada bit a bit para os consumidores atuais — é a mesma
 * implementação de `calcularTIRDetalhado`, sem o estado.
 */
export function calcularTIR(fluxos, opcoes = {}) {
  return calcularTIRDetalhado(fluxos, opcoes).valor_aa_pct
}
...
export function calcularFinanceiroCompleto({
  ...
  regulatorio = null,
  // FV-DOM-011C (E3): o instante é INJETÁVEL. `calculado_em` é metadado — não
  // participa de nenhuma conta, de nenhum hash e de nenhuma comparação. Deixá-lo
  // fixo em `new Date()` tornava duas execuções da mesma entrada diferentes
  // entre si, impedindo comparar resultados sem removê-lo à mão.
  // O default é o comportamento de antes, bit a bit.
  agora = null,
}) {
