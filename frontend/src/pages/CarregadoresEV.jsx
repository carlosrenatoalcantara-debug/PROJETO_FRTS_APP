...
import { Plus, Edit2, Trash2, ChevronDown, ChevronUp, Zap, Upload } from 'lucide-react'
...
function CardCarregadorEV({ carregador, onEditar, onExcluir }) {
  const [aberto, setAberto] = useState(false)
  const e = carregador.especificacoes || {}
    ...
    <Card className="overflow-hidden transition-shadow hover:shadow-md">
      <div className="flex items-center gap-3 px-5 py-4">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-bold text-slate-900 truncate">{carregador.fabricante} — {carregador.modelo}</p>
          <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-0.5 text-xs text-slate-500">
            {e.potencia_kw != null && <span className="font-semibold text-slate-700">{e.potencia_kw} kW</span>}
            {e.tipo_conector != null && <span>Conector: {e.tipo_conector}</span>}
            {e.voltagem != null && <span>{e.voltagem} V</span>}
            {e.corrente != null && <span>{e.corrente} A</span>}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button onClick={() => onEditar(carregador)} className="p-2 hover:bg-blue-50 rounded-lg text-blue-600 transition-colors">
            <Edit2 size={16} />
          </button>
          <button onClick={() => onExcluir(carregador._id)} className="p-2 hover:bg-red-50 rounded-lg text-red-500 transition-colors">
            <Trash2 size={16} />
          </button>
        </div>
      </div>
        ...
        <div className="border-t border-slate-100 bg-slate-50 px-5 py-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <Secao titulo="Especificações Técnicas">
              <Linha label="Potência" value={val(e.potencia_kw, 2, 'kW')} />
              <Linha label="Tipo de Conector" value={val(e.tipo_conector, 0)} />
              <Linha label="Voltagem" value={val(e.voltagem, 1, 'V')} />
              <Linha label="Corrente Máxima" value={val(e.corrente, 1, 'A')} />
              <Linha label="Eficiência" value={val(e.eficiencia, 1, '%')} />
            </Secao>

            <Secao titulo="Características">
              <Linha label="Tipo de Instalação" value={val(e.tipo_instalacao, 0)} />
              <Linha label="Certificação" value={val(e.certificacao, 0)} />
            ...
            <Secao titulo="Garantias">
              ...
              />
              <Linha label="Cobertura" value={val(e.garantia_cobertura, 0)} />
            </Secao>
          </div>
        </div>
      )}
...
export default function CarregadoresEV() {
  ...
  function handleNovo() { setCarregadorEditar(null); setModalAberto(true) }
  function handleEditar(c) { setCarregadorEditar(c); setModalAberto(true) }
  function handleSalvar() { setModalAberto(false); carregarCarregadores() }
  ...
  function onDrop(e) {
    ...
    if (pdfs.length) { setCarregadorEditar(null); setModalAberto(true) }
  }
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            Arraste datasheets ou clique em <strong>Novo Carregador</strong> para importar
          </p>
        </div>
        <Button
          onClick={handleNovo}
        ...
        >
          ...
          {arrastando ? 'Soltar aqui' : 'Novo Carregador'}
        </Button>
      </div>
      ...
      {carregando ? (
        ...
        <Card><CardBody>
          <div className="text-center py-10 text-slate-400">
            ...
            <p className="font-medium text-slate-600">Nenhum carregador cadastrado</p>
            <p className="text-sm mt-1">Clique em <strong>Novo Carregador</strong> ou arraste um datasheet PDF</p>
          </div>
        </CardBody></Card>
      ) : (
        <div className="space-y-3">
          <p className="text-sm text-slate-500 font-medium">
            {carregadores.length} carregador{carregadores.length > 1 ? 'es' : ''} cadastrado{carregadores.length > 1 ? 's' : ''}
